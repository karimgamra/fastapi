...
#     # Fix the typo: 'user_dict' was undefined, should be 'student_dict'
...
# @app.post("/debug/refresh-teacher-ids")
# async def refresh_teacher_ids(current_user: dict = Depends(get_current_user)):
#     if current_user["role"] != "admin":
#         raise HTTPException(status_code=403, detail="Admin access required")
#     global valid_teacher_ids
#     valid_teacher_ids = {teacher["id"] for teacher in await teacher_info_collection.find({}, {"id": 1}).to_list(None)}
#     return {"message": "Teacher IDs refreshed", "valid_teacher_ids": list(valid_teacher_ids)}
...
# MONGO_URI = os.getenv("MONGO_URI", "mongodb://localhost:27017")
# client = AsyncIOMotorClient(MONGO_URI)
# db = client.get_database("auth_db")  # Updated to match your database name
# admins_collection = db.admins
# student_info_collection = db.student_info
# teacher_info_collection = db.teacher_info
# subjects_collection = db.subjects
...
# @app.get("/admin/students")
# async def get_students(speciality: Optional[str] = Query(None), admin: dict = Depends(get_admin_user)):
#     query = {}
#     if speciality:
#         query["speciality"] = {"$regex": f"^{speciality}$", "$options": "i"}  # Case-insensitive match

#     logger.debug(f"Querying students with query: {query}")
#     students = await student_info_collection.find(
#         query,
#         {"_id": 0, "id": 1, "username": 1, "email": 1, "speciality": 1}
#     ).to_list(100)
#     logger.debug(f"Found students: {students}")
    
#     if not students:
#         raise HTTPException(status_code=404, detail="No students found for this speciality")
#     return {"students": students}

# @app.get("/subjects")
# async def get_subjects(class_name: Optional[str] = Query(None)):
#     query = {}
#     if class_name:
...
# @app.get("/events")
# async def get_events(start_date: datetime, end_date: datetime, user_id: str, role: str, class_name: Optional[str] = Query(None)):
#     query = {
#         "start_time": {"$gte": start_date, "$lte": end_date}
#     }
...
#         if not student:
#             raise HTTPException(status_code=404, detail="Student not found")
#         query["class_name"] = student["speciality"]
#     elif role == "admin":
#         if not class_name:
#             raise HTTPException(status_code=400, detail="class_name is required for admin role")
#         query["class_name"] = class_name
#     else:
#         raise HTTPException(status_code=400, detail="Invalid role")
...
# async def startup_event():
#     await subjects_collection.create_index([("name", 1), ("teacher_id", 1), ("class_name", 1)], unique=True)
#     await attendance_collection.create_index([("student_id", 1), ("subject_id", 1), ("date", 1)], unique=True)
#     await student_info_collection.create_index([("id", 1)], unique=True)
#     await teacher_info_collection.create_index([("id", 1)], unique=True)
#     await events_collection.create_index([("subject_id", 1), ("start_time", 1)])
#     await events_collection.create_index([("teacher_id", 1), ("start_time", 1)])
...
#     query = {}
#     if speciality:
#         query["speciality"] = {"$regex": f"^{speciality}$", "$options": "i"}
...
#         return {"message": "Attendance marked successfully"}
#     except pymongo.errors.DuplicateKeyError:
#         raise HTTPException(status_code=400, detail="Attendance already marked for this student, subject, and date")
...
#     for record in attendance_records:
#         record["_id"] = str(record["_id"])
#         record.pop("event_id", None)  # Remove event_id if it exists in older records
#     return attendance_records
...
async def login_for_access_token(form_data: LoginForm):
    ...
    # Handle username field based on role
    username_field = "username" if role in ["student", "admin"] else "name"
...
@app.get("/student/absences", response_model=List[AbsenceSummary])
async def get_student_absences(current_user: dict = Depends(get_current_user)):
    ...
    # Fetch attendance records where status is "Absent"
    attendance_records = await attendance_collection.find({
        "student_id": student_id,
        "status": "Absent"
    }).to_list(None)
    
    # Group absences by subject_id
    absence_counts = {}
    for record in attendance_records:
        subject_id = record["subject_id"]
        if subject_id not in absence_counts:
            absence_counts[subject_id] = 0
        absence_counts[subject_id] += 1
    
    # Fetch subject details for each subject_id
    absence_summaries = []
    for subject_id, count in absence_counts.items():
        try:
            subject = await subjects_collection.find_one({"_id": ObjectId(subject_id)})
        except bson.errors.InvalidId:
            continue  # Skip invalid subject_ids
        
        if not subject:
            continue  # Skip if subject not found
        
        absence_summaries.append({
            "subject_id": subject_id,
            "subject_name": subject["name"],
            "absence_count": count
        })
    
    if not absence_summaries:
        return []
    
    return absence_summaries
...
@app.on_event("startup")
async def startup_event():
    # Create an index on the email field for faster lookups (optional)
